...
class App extends React.Component {
  ...
  deleteComment = (e, post, key) => {
    ...
    fetch(`http://localhost:3000/posts/${post.id}/`, reqObj)
      ...
      .then((updatedCommentObj) =>
        this.setState({
          comments: this.state.comments.map((comment) => comment.id),
        })
      );
  };
  ...
  componentDidMount() {
    fetch("http://localhost:3000/posts")
      .then((resp) => resp.json())
      .then((postData) =>
        this.setState({ posts: postData.sort((a, b) => b.id - a.id) })
      );
  }
  ...
  render() {
    ...
    let filteredFavorites = this.state.posts.filter(
      (post) => post.favorited !== null
    );

    let favoritedPosts = filteredFavorites.sort(
      (a, b) => new Date(b.favorited) - new Date(a.favorited)
    );
